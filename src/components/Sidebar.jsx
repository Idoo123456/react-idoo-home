...
const Sidebar = ({ onAddMenusClick }) => {
  const menuItems = [
    ...
    { id: 'orders', icon: FaShoppingBag, label: 'Orders', path: '/orders' },
    { id: 'customers', icon: FaHeadset, label: 'Customers', path: '/customers' },
  ];
            ...
            {menuItems.map((item) => (
              <li key={item.id}>
                <NavLink
                  to={item.path}
                  className={({ isActive }) =>
                    `w-full flex items-center gap-4 px-4 py-4 rounded-3xl transition ${
                      isActive
